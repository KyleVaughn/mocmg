...
class TestMakeGridMesh(TestCase):
    ...
    def test_make_gridmesh_1_level(self):  # noqa: C901
        """Test generating a GridMesh hierarchy from a Mesh with one grid level."""
        # Use the two pin model and just make L2 cells sets L1
        ...
        mesh = mocmg.mesh.Mesh(
            pin_1and2_vertices, pin_1and2_cells, pin_1and2_cell_sets_1_level, name="both_pins"
        )
        self.assertEqual(pin_1and2_vertices, mesh.vertices)
        self.assertEqual(pin_1and2_cells, mesh.cells)
        self.assertEqual(pin_1and2_cell_sets_1_level, mesh.cell_sets)
        self.assertEqual("both_pins", mesh.name)
        both_pins_mesh = mocmg.mesh.make_gridmesh(mesh)
        # Root level
        self.assertEqual(None, both_pins_mesh.vertices)
        ...
        self.assertEqual(None, both_pins_mesh.cell_sets)
        self.assertEqual("both_pins", both_pins_mesh.name)
        self.assertEqual(len(both_pins_mesh.children), 2)
        # level1
        level1_1_1, level1_2_1 = both_pins_mesh.children
        self.assertEqual(level1_1_1.name, "GRID_L1_1_1")
        self.assertEqual(level1_2_1.name, "GRID_L1_2_1")
        self.assertEqual(level1_1_1.parent.name, "both_pins")
        self.assertEqual(level1_2_1.parent.name, "both_pins")
        # level1_1_1
        # vertices
        vert_keys = list(pin_1_vertices.keys())
        for v in vert_keys:
            for i in range(3):
                self.assertEqual(level1_1_1.vertices[v][i], pin_1_vertices[v][i])
        # cells
        cell_types = list(pin_1_cells.keys())
        for cell_type in cell_types:
            cell_ids = list(pin_1_cells[cell_type].keys())
            for cell_id in cell_ids:
                ...
                for v in range(len(vert_ids)):
                    self.assertEqual(
                        level1_1_1.cells[cell_type][cell_id][v], pin_1_cells[cell_type][cell_id][v]
                    )
        # cell_sets
        for set_name in list(pin_1_cell_sets.keys()):
            set_cells = pin_1_cell_sets[set_name]
            for cell in range(len(set_cells)):
                self.assertEqual(
                    level1_1_1.cell_sets[set_name][cell], pin_1_cell_sets[set_name][cell]
                )
        # level1_2_1
        # vertices
        vert_keys = list(pin_2_vertices.keys())
        for v in vert_keys:
            for i in range(3):
                self.assertEqual(level1_2_1.vertices[v][i], pin_2_vertices[v][i])
        # cells
        cell_types = list(pin_2_cells.keys())
        for cell_type in cell_types:
            cell_ids = list(pin_2_cells[cell_type].keys())
            for cell_id in cell_ids:
                ...
                for v in range(len(vert_ids)):
                    self.assertEqual(
                        level1_2_1.cells[cell_type][cell_id][v], pin_2_cells[cell_type][cell_id][v]
                    )
        # cell_sets
        for set_name in list(pin_2_cell_sets.keys()):
            set_cells = pin_2_cell_sets[set_name]
            for cell in range(len(set_cells)):
                self.assertEqual(
                    level1_2_1.cell_sets[set_name][cell], pin_2_cell_sets[set_name][cell]
                )
