...
# 2. Get bounding box
    if bb is None:
        ...
        module_log.warning('Bounding box for rectangular grid manually specified.' +
...
# 4. Fragment the grid components with the original model components
    ...
    module_log.info(f'Fragmenting {len(modelDimTags)} entities' +
...
# 5. Assign old physical groups to new entities
    ...
    for tag in fragmentChildrenGroups.keys():
        ...
        if outTag != tag:
            module_log.warning(f'Physical group {tag} could not be assigned to' +
